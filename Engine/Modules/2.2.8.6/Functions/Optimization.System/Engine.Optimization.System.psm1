...
Function Optimization_System_UI
{
	...
	$UI_Main_Restore_Click = {
		$GUIOS.Hide()
		if ($UI_AI_Opt.Enabled) {
			if ($UI_AI_Opt.Checked) {
				if ($UI_Recall_AI.checked) { Recall_AI -Enabled }
				if ($UI_Paint_AI.checked) { Paint_AI -Enabled }
				if ($UI_Notepad_AI.checked) { Notepad_AI -Enabled }
			}
		}

		if ($UI_Browser.Enabled) {
			if ($UI_Browser.Checked) {
				if ($UI_Edge_PD.Checked) { Edge_PD -Enabled }
				if ($UI_Edge_Ads.Checked) { Edge_Ads -Enabled }
				if ($UI_Edge_AI.Checked) { Edge_AI -Enabled }
			}
		}

		if ($UI_Main_Optimiz_Other_Is.Enabled) {
			if ($UI_Main_Optimiz_Other_Is.Checked) {
				if ($GUI_dgreadiness.Checked) { Win11_Dgreadiness -Enabled }
				if ($GUI_Spotlight.Checked) { Spotlight -Enabled }
				if ($GUI_ClickToDo.Checked) { ClickToDo -Enabled }
				if ($GUI_Copilot.Checked) { Copilot -Enabled }
				if ($GUI_DldlcTips.Checked) { DldlcTips -Enabled }
				if ($GUI_MS365ADS.Checked) { MS365ADS -Enabled }
		...
		if ($GUIExplorer.Enabled) {
			if ($GUIExplorer.Checked) {
				if ($GUIDiscoverFolderTypes.Checked) { Discover_Folder_Types -Enabled }
				if ($GUIDrdfnpofe.Checked) { Drdfnpofe -Enabled }
		...
		if ($GUIStart.Enabled) {
			if ($GUIStart.Checked) {
				...
				if ($GUITaskbarEnd_Task.Checked) { End_Task -Add }
		...
		if ($GUIPrivate.Enabled) {
			if ($GUIPrivate.Checked) {
				...
				if ($GUIOnlineSR.Checked) { OnlineSR -Enabled }
				if ($Start_TrackProgs.Checked) { Start_TrackProgs -Enabled }
	...
	$UI_Main_OK_Click = {
		...
		if ($UI_AI_Opt.Enabled) {
			if ($UI_AI_Opt.Checked) {
				if ($UI_Recall_AI.checked) { Recall_AI -Disable }
				if ($UI_Paint_AI.checked) { Paint_AI -Disable }
				if ($UI_Notepad_AI.checked) { Notepad_AI -Disable }
			}
		}

		if ($UI_Browser.Enabled) {
			if ($UI_Browser.Checked) {
				if ($UI_Edge_Is.Checked) { ForceRemoveEdge }
				if ($UI_Edge_PD.Checked) { Edge_PD -Disable }
				if ($UI_Edge_Ads.Checked) { Edge_Ads -Disable }
				if ($UI_Edge_AI.Checked) { Edge_AI -Disable }
			}
		}

		if ($UI_Main_Optimiz_Other_Is.Enabled) {
			if ($UI_Main_Optimiz_Other_Is.Checked) {
				if ($GUI_dgreadiness.Checked) { Win11_Dgreadiness -Disable }
				if ($GUI_Spotlight.Checked) { Spotlight -Disable }
				if ($GUI_ClickToDo.Checked) { ClickToDo -Disable }
				if ($GUI_Copilot.Checked) { Copilot -Disable }
				if ($GUI_DldlcTips.Checked) { DldlcTips -Disable }
				if ($GUI_MS365ADS.Checked) { MS365ADS -Disable }
		...
		if ($GUIExplorer.Enabled) {
			if ($GUIExplorer.Checked) {
				if ($GUIDiscoverFolderTypes.Checked) { Discover_Folder_Types -Disable }
				if ($GUIDrdfnpofe.Checked) { Drdfnpofe -Disable }
		...
		if ($GUIStart.Enabled) {
			if ($GUIStart.Checked) {
				...
				if ($GUITaskbarEnd_Task.Checked) { End_Task -Remove }
		...
		if ($GUIPrivate.Enabled) {
			if ($GUIPrivate.Checked) {
				...
				if ($GUIOnlineSR.Checked) { OnlineSR -Disable }
				if ($Start_TrackProgs.Checked) { Start_TrackProgs -Disable }
	...
	$UI_AI_Opt         = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_click      = {
			if ($UI_AI_Opt.Checked) {
				$UI_AI_Opt_Panel.Enabled = $True
			} else {
				$UI_AI_Opt_Panel.Enabled = $False
			}
		}
	}
	$UI_AI_Opt_Panel = New-Object system.Windows.Forms.FlowLayoutPanel -Property @{
		BorderStyle    = 0
		autoSizeMode   = 1
		autoSize       = 1
		autoScroll     = $False
	...
	$UI_Browser        = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_click      = {
			if ($UI_Browser.Checked) {
				$UI_Browser_Panel.Enabled = $True
			} else {
				$UI_Browser_Panel.Enabled = $False
			}
		}
	}
	$UI_Browser_Panel = New-Object system.Windows.Forms.FlowLayoutPanel -Property @{
		BorderStyle    = 0
		autoSizeMode   = 1
		autoSize       = 1
		autoScroll     = $False
	...
	if (Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion" -Name "CurrentBuild" -ErrorAction SilentlyContinue) {
		$GetLanguagePrompt = Get-ItemPropertyValue -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion" -Name "CurrentBuild" -ErrorAction SilentlyContinue
		if ($GetLanguagePrompt -lt 22000) {
			$UI_AI_Opt_Panel.controls.AddRange($UI_Recall_AI)
		}

		if ($GetLanguagePrompt -ge 22621) {
			$UI_AI_Opt_Panel.controls.AddRange((
				$UI_Paint_AI,
				$UI_Notepad_AI
			))
		}
	...
	$UI_Browser_Panel.controls.AddRange((
		$UI_Edge_Is,
		$UI_Edge_PD
	))
	if (Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion" -Name "CurrentBuild" -ErrorAction SilentlyContinue) {
...
Function Recall_AI
{
	...
	write-host "  $($lang.RecallAI)"
	if ($Enabled) {
		Remove-ItemProperty -Path "HKCU:\Software\Policies\Microsoft\Windows\WindowsAI" -Name "DisableAIDataAnalysis" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\Software\Policies\Microsoft\Windows\WindowsAI" -Name "DisableAIDataAnalysis" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\Software\Policies\Microsoft\Windows\WindowsAI" -Name "AllowRecallEnablement" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\Software\Policies\Microsoft\Windows\WindowsAI" -Name "TurnOffSavingSnapshots" -ErrorAction SilentlyContinue | Out-Null
		write-host "  $($lang.Inoperable)`n" -ForegroundColor Red
	}

	if ($Disable) {
		write-host "  $($lang.Disable)".PadRight(22) -NoNewline
		if ((Test-Path -LiteralPath "HKCU:\Software\Policies\Microsoft\Windows\WindowsAI") -ne $true) { New-Item "HKCU:\Software\Policies\Microsoft\Windows\WindowsAI" -force -ErrorAction SilentlyContinue | Out-Null }
		if ((Test-Path -LiteralPath "HKLM:\Software\Policies\Microsoft\Windows\WindowsAI") -ne $true) { New-Item "HKLM:\Software\Policies\Microsoft\Windows\WindowsAI" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -Path "HKCU:\Software\Policies\Microsoft\Windows\WindowsAI" -Name "DisableAIDataAnalysis" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\Software\Policies\Microsoft\Windows\WindowsAI" -Name "DisableAIDataAnalysis" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\Software\Policies\Microsoft\Windows\WindowsAI" -Name "AllowRecallEnablement" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\Software\Policies\Microsoft\Windows\WindowsAI" -Name "TurnOffSavingSnapshots" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null
...
Function Paint_AI
{
	...
	write-host "  $($lang.PaintAI)"
	if ($Enabled) {
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Paint" -Name "DisableCocreator" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Paint" -Name "DisableGenerativeFill" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Paint" -Name "DisableImageCreator" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Paint" -Name "DisableGenerativeErase" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Paint" -Name "DisableRemoveBackground" -ErrorAction SilentlyContinue | Out-Null
		write-host "  $($lang.Inoperable)`n" -ForegroundColor Red
	}

	if ($Disable) {
		write-host "  $($lang.Disable)".PadRight(22) -NoNewline
		if ((Test-Path -LiteralPath "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Paint") -ne $true) { New-Item "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Paint" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Paint" -Name "DisableCocreator" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Paint" -Name "DisableGenerativeFill" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Paint" -Name "DisableImageCreator" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Paint" -Name "DisableGenerativeErase" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Paint" -Name "DisableRemoveBackground" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}
}
...
Function Notepad_AI
{
	write-host "  $($lang.NotepadAI)"
	if ($Enabled) {
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\WindowsNotepad" -Name "DisableAIFeatures" -ErrorAction SilentlyContinue | Out-Null
		write-host "  $($lang.Inoperable)`n" -ForegroundColor Red
	}

	if ($Disable) {
		write-host "  $($lang.Disable)".PadRight(22) -NoNewline
		if ((Test-Path -LiteralPath "HKLM:\SOFTWARE\Policies\WindowsNotepad") -ne $true) { New-Item "HKLM:\SOFTWARE\Policies\WindowsNotepad" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\WindowsNotepad" -Name "DisableAIFeatures" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}
}
...
Function ForceRemoveEdge
{
	...
	New-Item $edgeStub -ItemType Directory -ErrorAction SilentlyContinue | Out-Null
	New-Item "$($edgeStub)\MicrosoftEdge.exe" -ErrorAction SilentlyContinue | Out-Null
	...
	if ($null -ne $uninstallRegKey) {
		...
		$valuesToRemove = @(
			'MicrosoftEdgeAutoLaunch_A9F6DCE4ABADF4F51CF45CD7129E3C6C',
			'Microsoft Edge Update'
		)

		foreach ($value in $valuesToRemove) {
			Remove-ItemProperty -Path $runPath -Name $value -ErrorAction SilentlyContinue | out-null
			Remove-ItemProperty -Path $startupApprovedPath -Name $value -ErrorAction SilentlyContinue | out-null
		}

		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	} else {
		Write-Host "$($lang.Failed)`n" -ForegroundColor Red
	}
}
...
Function Edge_PD
{
	...
	write-host "  $($lang.EdgePD)"
	if ($Enabled) {
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "PersonalizationReportingEnabled" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "DiagnosticData" -ErrorAction SilentlyContinue | Out-Null
		write-host "  $($lang.Inoperable)`n" -ForegroundColor Red
	}
	...
	if ($Disable) {
		...
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "PersonalizationReportingEnabled" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "DiagnosticData" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}
}
...
Function Edge_Ads
{
	...
	write-host "  $($lang.EdgeAds)"
	if ($Enabled) {
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "NewTabPageContentEnabled" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "NewTabPageHideDefaultTopSites" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "EdgeShoppingAssistantEnabled" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "TabServicesEnabled" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "AlternateErrorPagesEnabled" -ErrorAction SilentlyContinue | Out-Null
		write-host "  $($lang.Inoperable)`n" -ForegroundColor Red
	}
	...
	if ($Disable) {
		write-host "  $($lang.Disable)".PadRight(22) -NoNewline
		if ((Test-Path -LiteralPath "HKLM:\SOFTWARE\Policies\Microsoft\Edge") -ne $true) { New-Item "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "NewTabPageContentEnabled" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "NewTabPageHideDefaultTopSites" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "EdgeShoppingAssistantEnabled" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "TabServicesEnabled" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "AlternateErrorPagesEnabled" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}
}
...
Function Edge_AI
{
	...
	write-host "  $($lang.EdgeAI)"
	if ($Enabled) {
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "CopilotCDPPageContext" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "CopilotPageContext" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "HubsSidebarEnabled" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "EdgeEntraCopilotPageContext" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "EdgeHistoryAISearchEnabled" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "ComposeInlineEnabled" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "GenAILocalFoundationalModelSettings" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "NewTabPageBingChatEnabled" -ErrorAction SilentlyContinue | Out-Null
		write-host "  $($lang.Inoperable)`n" -ForegroundColor Red
	}
	...
	if ($Disable) {
		write-host "  $($lang.Disable)".PadRight(22) -NoNewline
		if ((Test-Path -LiteralPath "HKLM:\SOFTWARE\Policies\Microsoft\Edge") -ne $true) { New-Item "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "CopilotCDPPageContext" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "CopilotPageContext" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "HubsSidebarEnabled" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "EdgeEntraCopilotPageContext" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "EdgeHistoryAISearchEnabled" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "ComposeInlineEnabled" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "GenAILocalFoundationalModelSettings" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Edge" -Name "NewTabPageBingChatEnabled" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}
}
...
Function Copilot
{
	...
	write-host "  $($lang.Copilot)"
	if ($Enabled) {
		New-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "ShowCopilotButton" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null
	...
	if ($Disable) {
		write-host "  $($lang.Disable)".PadRight(22) -NoNewline
		New-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "ShowCopilotButton" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null

		if ((Test-Path -LiteralPath "HKCU:\SOFTWARE\Policies\Microsoft\Windows\WindowsCopilot") -ne $true) { New-Item "HKCU:\SOFTWARE\Policies\Microsoft\Windows\WindowsCopilot" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -Path "HKCU:\SOFTWARE\Policies\Microsoft\Windows\WindowsCopilot" -Name "TurnOffWindowsCopilot" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null

		if ((Test-Path -LiteralPath "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsCopilot") -ne $true) { New-Item "HKLM:\SOFTWARE\Policies\Microsoft\Windows\WindowsCopilot" -force -ErrorAction SilentlyContinue | Out-Null }
...
Function ClickToDo
{
	...
	write-host "  $($lang.ClickToDo)"
	if ($Enabled) {
		Remove-ItemProperty -Path "HKCU:\Software\Policies\Microsoft\Windows\WindowsAI" -Name "DisableClickToDo" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKLM:\Software\Policies\Microsoft\Windows\WindowsAI" -Name "DisableClickToDo" -ErrorAction SilentlyContinue | Out-Null
		write-host "  $($lang.Inoperable)`n" -ForegroundColor Red
	}
	...
	if ($Disable) {
		write-host "  $($lang.Disable)".PadRight(22) -NoNewline
		if ((Test-Path -LiteralPath "HKCU:\Software\Policies\Microsoft\Windows\WindowsAI") -ne $true) { New-Item "HKCU:\Software\Policies\Microsoft\Windows\WindowsAI" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -Path "HKCU:\Software\Policies\Microsoft\Windows\WindowsAI" -Name "DisableClickToDo" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null

		if ((Test-Path -LiteralPath "HKLM:\Software\Policies\Microsoft\Windows\WindowsAI") -ne $true) { New-Item "HKLM:\Software\Policies\Microsoft\Windows\WindowsAI" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -Path "HKLM:\Software\Policies\Microsoft\Windows\WindowsAI" -Name "DisableClickToDo" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null
...
Function Spotlight
{
	...
	write-host "  $($lang.Spotlight)"
	if ($Enabled) {
		Remove-ItemProperty -Path "HKCU:\Software\Policies\Microsoft\Windows\CloudContent" -Name "DisableSpotlightCollectionOnDesktop" -ErrorAction SilentlyContinue | Out-Null
		write-host "  $($lang.Inoperable)`n" -ForegroundColor Red
	}

	if ($Disable) {
		write-host "  $($lang.Disable)".PadRight(22) -NoNewline
		if ((Test-Path -LiteralPath "HKCU:\Software\Policies\Microsoft\Windows\CloudContent") -ne $true) { New-Item "HKCU:\Software\Policies\Microsoft\Windows\CloudContent" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -Path "HKCU:\Software\Policies\Microsoft\Windows\CloudContent" -Name "DisableSpotlightCollectionOnDesktop" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}
}
...
Function DldlcTips
{
	...
	write-host "  $($lang.DldlcTips)"
	if ($Enabled) {
		New-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\ContentDeliveryManager" -Name "SubscribedContent-338387Enabled" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\ContentDeliveryManager" -Name "RotatingLockScreenOverlayEnabled" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null
		write-host "  $($lang.Inoperable)`n" -ForegroundColor Red
	}

	if ($Disable) {
		write-host "  $($lang.Disable)".PadRight(22) -NoNewline
		New-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\ContentDeliveryManager" -Name "SubscribedContent-338387Enabled" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
		New-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\ContentDeliveryManager" -Name "RotatingLockScreenOverlayEnabled" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}
}
...
Function MS365ADS
{
	...
	write-host "  $($lang.MS365ADS)"
	if ($Enabled) {
		if ((Test-Path -LiteralPath "HKLM:\Software\Policies\Microsoft\Windows\CloudContent") -ne $true) { New-Item "HKLM:\Software\Policies\Microsoft\Windows\CloudContent" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\CloudContent" -Name "DisableConsumerAccountStateContent" -PropertyType DWord -Value 0 -Force -ErrorAction SilentlyContinue | Out-Null
		write-host "  $($lang.Inoperable)`n" -ForegroundColor Red
	}

	if ($Disable) {
		write-host "  $($lang.Disable)".PadRight(22) -NoNewline
		New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\CloudContent" -Name "DisableConsumerAccountStateContent" -PropertyType DWord -Value 1 -Force -ErrorAction SilentlyContinue | Out-Null
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}
}
...
Function Win11_Dgreadiness
{
	...
	write-host "  $($lang.Dgreadiness)"
	$DgreadinessPS = Join-Path -Path "$($PSScriptRoot)\..\..\..\..\AIO\Dgreadiness" -ChildPath "DG_Readiness_Tool_v3.6.ps1"
	if (Test-Path -Path $DgreadinessPS -PathType leaf) {
		if ($Enabled) {
			write-host "  $($lang.Enable)".PadRight(22) -NoNewline
...
#>
<#
	.Auto discover folder types in File Explorer
	.自动文件夹类型发现
#>
Function Discover_Folder_Types
{
	param
	(
		[switch]$Enabled,
	...
	write-host "  $($lang.DiscoverFolderTypes)"
	if ($Enabled) {
		write-host "  $($lang.Enable)".PadRight(22) -NoNewline
		New-ItemProperty -Path "HKCU:\SOFTWARE\Classes\Local Settings\Software\Microsoft\Windows\Shell\Bags\AllFolders\Shell" -Name "FolderType" -PropertyType String -Value "NotSpecified" -Force -ErrorAction SilentlyContinue | Out-Null
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}

	if ($Disable) {
		write-host "  $($lang.Disable)".PadRight(22) -NoNewline
		Remove-ItemProperty -Path "HKCU:\SOFTWARE\Classes\Local Settings\Software\Microsoft\Windows\Shell\Bags\AllFolders\Shell" -Name "FolderType" -ErrorAction SilentlyContinue | Out-Null
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}
}
...
#>
<#
	.Hiding duplicate removable drive entries from the File Explorer navigation pane
	.在文件资源管理器导航窗格中隐藏重复的可移动驱动器条目
#>
Function Drdfnpofe
{
	param
	...
	write-host "  $($lang.Drdfnpofe)"
	if ($Enabled) {
		write-host "  $($lang.Enable)".PadRight(22) -NoNewline
		New-Item -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Desktop\NameSpace\DelegateFolders\{F5FB2C77-0E2F-4A16-A381-3E560C68BC83}" -Force -ErrorAction SilentlyContinue | Out-Null
		Set-Item -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Desktop\NameSpace\DelegateFolders\{F5FB2C77-0E2F-4A16-A381-3E560C68BC83}" -Value "Removable Drives" -ErrorAction SilentlyContinue | Out-Null
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}

	if ($Disable) {
		write-host "  $($lang.Disable)".PadRight(22) -NoNewline
		Remove-Item -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Desktop\NameSpace\DelegateFolders\{F5FB2C77-0E2F-4A16-A381-3E560C68BC83}" -force -Recurse -ErrorAction SilentlyContinue | Out-Null
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}
}
...
Function End_Task
{
	...
	write-host "  $($lang.End_Task)"
	if ($Add) {
		write-host "  $($lang.AddTo)".PadRight(22) -NoNewline
		if ((Test-Path -LiteralPath "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced\TaskbarDeveloperSettings") -ne $true) { New-Item "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced\TaskbarDeveloperSettings" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -LiteralPath 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced\TaskbarDeveloperSettings' -Name 'TaskbarEndTask' -Value 0 -PropertyType DWord -force -ErrorAction SilentlyContinue | Out-Null
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}
	...
	if ($Remove) {
		write-host "  $($lang.Del)".PadRight(22) -NoNewline
		if ((Test-Path -LiteralPath "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced\TaskbarDeveloperSettings") -ne $true) { New-Item "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced\TaskbarDeveloperSettings" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -LiteralPath 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced\TaskbarDeveloperSettings' -Name 'TaskbarEndTask' -Value 1 -PropertyType DWord -force -ErrorAction SilentlyContinue | Out-Null
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}
}
...
Function Bing_Search
{
	...
	if ($Enabled) {
		...
		Remove-ItemProperty -Path "HKCU:\SOFTWARE\Policies\Microsoft\Windows\Windows Search" -Name AllowCortana -Force -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKCU:\SOFTWARE\Policies\Microsoft\Windows\Windows Search" -Name CortanaConsent -Force -ErrorAction SilentlyContinue | Out-Null
		Restart_Explorer
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}
	...
	if ($Disable) {
		...
		New-ItemProperty -Path "HKCU:\SOFTWARE\Policies\Microsoft\Windows\Windows Search" -Name AllowCortana -PropertyType DWord -Value 0 -Force | Out-Null
		New-ItemProperty -Path "HKCU:\SOFTWARE\Policies\Microsoft\Windows\Windows Search" -Name CortanaConsent -PropertyType DWord -Value 0 -Force | Out-Null
...
Function Taskbar_Suggested_Content
{
	...
	$cdm = @(
		"Start_AccountNotifications"       # Disable Show account-related notifications
		"ShowSyncProviderNotifications"    # Sync provider ads
		"SubscribedContent-314559Enabled"
		"SilentInstalledAppsEnabled"       # Automatic Installation of Suggested Apps
		"SubscribedContent-338389Enabled"  # Get tips, tricks, and suggestions as you use Windows
		"SoftLandingEnabled"               # Get tips, tricks, and suggestions as you use Windows
		"SubscribedContent-338393Enabled"  # Show me suggested content in the Settings app
		"SubscribedContent-353694Enabled"  # Show me suggested content in the Settings app
		"SubscribedContent-353696Enabled"  # Show me suggested content in the Settings app
		"SubscribedContent-353698Enabled"  # Show me suggested content in the Settings app
		"Start_IrisRecommendations"        # Show recommendations for tips, shortcuts, new apps, and more in start
		...
		"SubscribedContent-338387Enabled"  # Show me suggested content in the Settings app
		"SubscribedContentEnabled"
		"SubscribedContent-338388Enabled"  # Occasionally show suggestions in Start
		"SystemPaneSuggestionsEnabled"     # Occasionally show suggestions in Start
	)
	...
	if ($Show) {
		...
		Remove-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\SystemSettings\AccountNotifications" -Name "EnableAccountNotifications" -ErrorAction SilentlyContinue | Out-Null
		...
		Remove-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Notifications\Settings\Windows.SystemToast.Suggested" -Name "Enabled" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Mobility" -Name "OptedIn" -ErrorAction SilentlyContinue | Out-Null
		Remove-ItemProperty -Path "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Notifications\Settings\Windows.SystemToast.BackupReminder" -Name "Enabled" -ErrorAction SilentlyContinue | Out-Null
	...
	if ($Hide) {
		...
		if ((Test-Path -LiteralPath "HKCU:\Software\Microsoft\Windows\CurrentVersion\SystemSettings\AccountNotifications") -ne $true) { New-Item "HKCU:\Software\Microsoft\Windows\CurrentVersion\SystemSettings\AccountNotifications" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -LiteralPath 'HKCU:\Software\Microsoft\Windows\CurrentVersion\SystemSettings\AccountNotifications' -Name 'EnableAccountNotifications' -Value 0 -PropertyType DWord -force -ErrorAction SilentlyContinue | Out-Null

		# Suggest ways I can finish setting up my device to get the most out of Windows
		if ((Test-Path -LiteralPath "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\UserProfileEngagement") -ne $true) { New-Item "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\UserProfileEngagement" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -LiteralPath 'HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\UserProfileEngagement' -Name 'ScoobeSystemSettingEnabled' -Value 0 -PropertyType DWord -force -ErrorAction SilentlyContinue | Out-Null

		# Disable "Suggested" app notifications (Ads for MS services)
		if ((Test-Path -LiteralPath "HKCU:\Software\Microsoft\Windows\CurrentVersion\Notifications\Settings\Windows.SystemToast.Suggested") -ne $true) { New-Item "HKCU:\Software\Microsoft\Windows\CurrentVersion\Notifications\Settings\Windows.SystemToast.Suggested" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -LiteralPath 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Notifications\Settings\Windows.SystemToast.Suggested' -Name 'Enabled' -Value 0 -PropertyType DWord -force -ErrorAction SilentlyContinue | Out-Null

		# Disable Show me suggestions for using my mobile device with Windows (Phone Link suggestions)
		if ((Test-Path -LiteralPath "HKCU:\Software\Microsoft\Windows\CurrentVersion\Mobility") -ne $true) { New-Item "HKCU:\Software\Microsoft\Windows\CurrentVersion\Mobility" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -LiteralPath 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Mobility' -Name 'OptedIn' -Value 0 -PropertyType DWord -force -ErrorAction SilentlyContinue | Out-Null

		# Disable Windows Backup reminder notifications
		if ((Test-Path -LiteralPath "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Notifications\Settings\Windows.SystemToast.BackupReminder") -ne $true) { New-Item "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Notifications\Settings\Windows.SystemToast.BackupReminder" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -LiteralPath 'HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Notifications\Settings\Windows.SystemToast.BackupReminder' -Name 'Enabled' -Value 0 -PropertyType DWord -force -ErrorAction SilentlyContinue | Out-Null
...
Function OnlineSR
{
	...
	write-host "  $($lang.OnlineSR)"
	if ($Enabled) {
		write-host "  $($lang.Enable)".PadRight(22) -NoNewline
		New-ItemProperty -LiteralPath 'HKCU:\Software\Microsoft\Speech_OneCore\Settings\OnlineSpeechPrivacy' -Name 'HasAccepted' -Value 1 -PropertyType DWord -force -ErrorAction SilentlyContinue | Out-Null
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}

	if ($Disable) {
		write-host "  $($lang.Disable)".PadRight(22) -NoNewline
		if ((Test-Path -LiteralPath "HKCU:\Software\Microsoft\Speech_OneCore\Settings\OnlineSpeechPrivacy") -ne $true) { New-Item "HKCU:\Software\Microsoft\Speech_OneCore\Settings\OnlineSpeechPrivacy" -force -ErrorAction SilentlyContinue | Out-Null }
		New-ItemProperty -LiteralPath 'HKCU:\Software\Microsoft\Speech_OneCore\Settings\OnlineSpeechPrivacy' -Name 'HasAccepted' -Value 0 -PropertyType DWord -force -ErrorAction SilentlyContinue | Out-Null
		Write-Host "$($lang.Done)`n" -ForegroundColor Green
	}
}
