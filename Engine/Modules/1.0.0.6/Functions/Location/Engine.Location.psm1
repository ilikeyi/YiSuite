			...
			if ($_ -is [System.Windows.Forms.RadioButton]) {
				if ($_.Checked) {
					...
					if ($GUILocationCustomize.Checked) {
						...
						Save_Dynamic -regkey "Suite\UserFolder" -name "Folder" -value "$($GUILocationCustomizeShow.Text)" -String
					}
	...
	$GUILocationCustomize = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			if ($GUILocationCustomize.Checked) {
				...
				Save_Dynamic -regkey "Suite\UserFolder" -name "AllowCustomize" -value "True" -String
			} else {
