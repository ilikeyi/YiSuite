...
<#
  ...
  .压缩包保存到
#>
$UpdateSaveTo = "$([Environment]::GetFolderPath("Desktop"))\YiSuite.Upgrade.Package"

<#
	.Archive temporary directory
	.压缩包临时目录
#>
$TempFolderUpdate = "$([Environment]::GetFolderPath("MyDocuments"))\Temp.YiSuite.Upgrade.Package"
	...
	$UI_Main_Create_ASC = New-Object System.Windows.Forms.CheckBox -Property @{
		...
		add_Click      = {
			if ($UI_Main_Create_ASC.Checked) {
				...
				Save_Dynamic -regkey "Suite" -name "IsPGP" -value "True" -String
			} else {
				$UI_Main_Create_ASC_Panel.Enabled = $False
				Save_Dynamic -regkey "Suite" -name "IsPGP" -value "False" -String
			}
		}
	}
			...
			if ($UI_Main_Create_ASC.Enabled) {
				if ($UI_Main_Create_ASC.Checked) {
					if ([string]::IsNullOrEmpty($UI_Main_Create_ASCSign.Text)) {
						...
						Save_Dynamic -regkey "Suite" -name "PGP" -value $UI_Main_Create_ASCSign.Text -String
